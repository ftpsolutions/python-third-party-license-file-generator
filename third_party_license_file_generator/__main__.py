...
if __name__ == "__main__":
    ...
    third_party_licenses += [
        "Start of {0} generated by Python third_party_license_generator at {1}".format(
    ...
    for _, module in sorted(joined.modules_by_module_name.items()):
        ...
        blurb += "{0}".format(module.license_file)

        third_party_licenses += [blurb.strip()]

    third_party_licenses += [
        "End of {0} generated by Python third_party_license_generator at {1}".format(
            repr(os.path.split(args.output_file)[-1]), datetime.datetime.now()
        )
    ]
